class Admin::PagesController < ApplicationController
  ...
  # GET /pages.xml
  def index
    @pages = Page.where(:site_id => uber_page_site.id ).all
  ...
  def create
    ...
    @page.name = Page.clean_name(@page.name)
    @page.site_id = uber_page_site.id
    respond_to do |format|
      if @page.save
  ...
  def update
    @page = Page.find(params[:id])
    params[:page][:name] = Page.clean_name(params[:page][:name])
    respond_to do |format|
